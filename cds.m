...
bw=input('enter tansmittion range of a node');
ax=[0 10 0 10];
axis(ax);
hold on;
axis(axis);
...
for i=1:n
    for j=i+1:n
        if(sqrt((s(i).x-s(j).x)^2+(s(i).y-s(j).y)^2)<bw)
                A(i,j)=1;
                A(j,i)=1;
                cnt(i)=cnt(i)+1;
...
for i=1:n
    if(s(i).head==1)
        plot(s(i).x,s(i).y,'r.');
        circle(s(i).x,s(i).y,bw);
    else
        plot(s(i).x,s(i).y,'b.');
    end
end
[a b]=ginput(1);
set(title_name,'String','connected dominated set is made');
for i=1:n
    for j=i:n
